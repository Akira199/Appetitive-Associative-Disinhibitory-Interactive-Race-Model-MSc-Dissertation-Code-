...
GOUnit = []; GOUnit(1:trialLength) = NaN; GOUnit(1) = 0;   % GO unit
STOPUnit = []; STOPUnit(1:trialLength) = NaN; STOPUnit(1) = 0;  % STOP unit

% Accumulation of activation in the GO and STOP units set up based on the
% cue-initiated model activation

if cueType == 0
    Unit_NeutralActivationCode; 
elseif cueType == 1
    Unit_AppetitiveActivationCode; 
end 
       
% GO and STOP RT measured relative to beginning of trial, after targ onset
moveth = []; fixth = [];
moveth = (find(GOUnit >= threshMove1));
fixth = (find(STOPUnit >= threshFix));
