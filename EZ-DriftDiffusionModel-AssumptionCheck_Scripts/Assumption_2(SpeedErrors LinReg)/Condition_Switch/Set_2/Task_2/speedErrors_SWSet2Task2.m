...
SET_2 = Final_SwitchPicData_switching(Final_SwitchPicData_switching.set == 'plane_number',:);
task_2Set2 = SET_2(SET_2.task == 'number',:);

%% Calculating meanRTs - correct RT's dataset:
idxSs = table();
idxRT = table();
meanRTCor_St2Task2 = cell(4,31);
for mm = 1:31
       idxSs = task_2Set2(task_2Set2.code == mm,:);
       for zz = 5:8 % this idx vals corresponds with the session idx
       idxRT = idxSs(idxSs.session == zz,{'rt'});
       rt = table2array(idxRT);
       calcMean = mean(rt);
       meanRTCor_St2Task2{zz,mm} = calcMean;
       end
end
...
task_2Set2Incor = SET_2Incorr(SET_2Incorr.task == 'number',:);

% Calculating meanRTs - Incorrect RT's dataset:
idxSsIncor_St1Task1 = table();
idxRTIncor_St1Task1 = table();
meanRTIncor_St2Task2= cell(4,31);
for m = 1:31
       idxSsIncor_St1Task1 = task_2Set2Incor(task_2Set2Incor.code == m,:);
       for z = 5:8
       ...
       meanRTIncor_St2Task2{z,m} = calcMeanIncorr;
       end
end

%% Convert all Incor MEAN RT values to NaN if the corresponding values for Cor MEAN RT is NaN:

meanRTCor_St2Task2 = cell2mat(meanRTCor_St2Task2);
meanRTIncor_St2Task2 = cell2mat(meanRTIncor_St2Task2);
for hh = 1:124 %the idx range has to be 4*31  
  if isnan(meanRTCor_St2Task2(hh))
     meanRTIncor_St2Task2(hh) = NaN;
  end 
end

%% Compile mean rt data from correct and incorrect set:
compile_MeanRts = [meanRTCor_St2Task2;meanRTIncor_St2Task2];
...
condition =  repmat({'switch','switch','switch','switch',...
                     'switch','switch','switch','switch'},1,26)';
 
set = repmat({'plane_number','plane_number','plane_number','plane_number',...
              'plane_number','plane_number','plane_number','plane_number'},1,26)';

task = repmat({'number','number','number','number',...
              'number','number','number','number',},1,26)';

stimuli = repmat({'pictorial','pictorial','pictorial','pictorial',...
                  'pictorial','pictorial','pictorial','pictorial',},1,26)';
              
MeanRT = Fin_MeanRt;

% Assemble the final dataset: 
Set2_task2_switchingPic = table(code,session,condition,set,task,stimuli,response,MeanRT,...
...
Set2_task2_switchingPic.Response = categorical(response);
model_4Set2Task2 = fitlm(Set2_task2_switchingPic,'MeanRT~session*Response')
model_4ANOVA_Set2Task2 = anova(model_4Set2Task2,'summary')
...
subplot(2,4,1)
plotResiduals(model_4Set2Task2,'probability','Color','red')
subplot(2,4,2)
plotResiduals(model_4Set2Task2,'fitted','Color','green')
subplot(2,4,3)
plotResiduals(model_4Set2Task2,'lagged','Color','blue')
subplot(2,4,4)
plotResiduals(model_4Set2Task2,'caseorder')
subplot(2,4,5)
plotResiduals(model_4Set2Task2,'symmetry')
subplot(2,4,6)
%plotInteraction(model_1Set1Task1,'session','Response','predictions')
plotInteraction(model_4Set2Task2,'Response','session','predictions')
subplot(2,4,7)
plot(model_4Set2Task2)